...
void GameSystem::CreateCharacter()
{
    ...
    // 建構並加入堆疊
    Character newCharacter(name, str, dex, con, job);
    this->_charactersList.push_back(newCharacter);
    cout << "創建成功!" << endl << endl << endl;
    // 輸出角色資料
    cout << newCharacter.GetInformation() << endl;
}
// 分配屬性點數
void GameSystem::DistributeAttributePoints(int point, int& str, int& dex, int& con, int extraStr, int extraDex, int extraCon)
{
    ...
    const string stringExtraStr = extraStr > 0 ? "+" + to_string(extraStr) : "";
    const string stringExtraDex = extraDex > 0 ? "+" + to_string(extraDex) : "";
    const string stringExtraCon = extraCon > 0 ? "+" + to_string(extraCon) : "";

    while (point)
    {
        // 顯示選單
        cout << endl << "1. 目前STR : " << str << stringExtraStr << endl;
        cout << "2. 目前DEX : " << dex << stringExtraDex << endl;
        cout << "3. 目前CON : " << con << stringExtraCon << endl;
